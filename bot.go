...
func NewBotAPI(token string, httpaddr string, wsaddr string) (*BotAPI, error) {
	return NewBotAPIWithClient(token, httpaddr, wsaddr, &websocket.Conn{}, &http.Client{})
}
...
//
// It requires a token, provided by @BotFather on Telegram.
func NewBotAPIWithClient(token string, httpaddr string, wsaddr string, conn *websocket.Conn, client *http.Client) (*BotAPI, error) {
	addr := flag.String("addr", wsaddr, "http service address")
	flag.Parse()
...
func (bot *BotAPI) MakeRequest(endpoint string, params url.Values) (APIResponse, error) {
	method := fmt.Sprintf(bot.HTTPAddr + APIEndpoint, endpoint, bot.Token)
...
func (bot *BotAPI) GetUpdates(config UpdateConfig) ([]Update, error) {
	...
	json.Unmarshal(message, &updatejson)
